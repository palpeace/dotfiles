...
- 一方で、AIを監督し環境を管理するための「ターミナルでの状況認識能力・機動力（Starship, Atuin, eza, bat, delta等のモダンCLI）」、成果物検証ツール（xh）、およびAIの環境基盤・操作補助（direnv, zsh補完）は、**人間のための必須インフラ（コマンドセンター）として積極的に保持**する。AIエージェントはこれらを「人間の古いプログラミング習慣」と誤認して削除提案をしてはならない。
