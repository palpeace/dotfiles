...
## 7. AIへの実装委譲 (Delegation to agy / Gemini Flash)
- 判断を伴わない実装作業（大量ファイルの走査・機械的な一括変換・テストやlintの緑化ループ）は `~/.local/bin/agy-run` 経由で Gemini Flash (agy) へ委譲し、Claude側の文脈消費を抑えてよい。設計判断・規約変更・ユーザーとの対話・不可逆操作（コミット・push・削除）は委譲しないこと。
- **agyの標準出力を直接読まない。** `agy-run` は結果を出力ディレクトリのファイルに落とし、標準出力には出力先パスだけを返す設計になっている。呼び出し側は `jq` 等で必要なフィールドだけを読み、生のstdoutをそのまま文脈に入れないこと。
- **agyの自己申告を成否判定に使わない。** 権限で拒否された時でも成功したかのような応答を返すことがあるため、受け入れはファイルの存在・`git diff`・テスト・lintなど呼び出し側の機械検証で行うこと。
- 使い方の詳細は `~/.claude/skills/agy-delegate/SKILL.md` を参照。モデルの優先順（どのモデルを何番目に使うか）は `agy-run` の引数として渡す可変値であり、ここには固定しない。
